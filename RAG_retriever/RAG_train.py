...
DEFAULT_DATASET_DIR = "/workspace/TAU-R1/dataset/train"
DEFAULT_OUTPUT_DIR = "/data/aicity_train"
...
def _run_retrieval(toolkit, question, api_key, gt_answer=None, gt_reasoning=None):
    ...
    model = ChatOpenAI(
        model="gpt-5.5",                     # or whatever model name ModelSell uses
        api_key=api_key,
        base_url="https://api.modelsell.com/v1",
        temperature=0.0,
    )
    agent = create_react_agent(model, [caption_retrieval, free_text_tracking, ground_truth_retrieval],
                               prompt=prompts.retrieval_system_prompt)
...
def process_video(job):
    ...
    try:
        ...
        for q in todo:
            ...
            task = q["task"]
            results_by_task.setdefault(task, []).append(_enrich(job["video_path"], entry))
...
# python RAG_retriever/RAG_train.py --gpus 0 --workers-per-gpu 24 --start 0 --end 2800
# vetinari python RAG_retriever/RAG_train.py --gpus 0 1 2 --workers-per-gpu 12 --start 252
# python RAG_retriever/RAG_train.py --gpus 0 1 --workers-per-gpu 6 --dataset-dir /workspace/TAU-R1/dataset/test/PSI_VQA --tasks bcq mcq open_qa temporal_localization --output-dir /data/PSI_VQA_test_rag_stage1

# python RAG_retriever/RAG_train.py --gpus 0 1 --workers-per-gpu 24 --dataset-dir /workspace/TAU-R1/dataset/train