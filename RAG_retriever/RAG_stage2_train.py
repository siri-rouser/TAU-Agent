...
This retrieval is computed once per video (it does not depend on task), then the
same factual/potential/frame_range info is used to enrich EVERY task's JSON for
that video (all 9 training tasks), mirroring the layout of
/mnt/fast0/yl4300/aicity_track3_video/aicity_train_2stage:
    <output_dir>/<task>/<dataset>/.../<video>.json

Outputs
-------
  <output_dir>/_raw_cache/<video_id_no_ext>.json        parsed retrieval (resumability marker)
      fields: factual_information, potential_information, frame_ranges
  <output_dir>/<task>/<video_id_no_ext>.json            enriched copy of the matching
      <stage1_rag_dir>/<task>/<video_id_no_ext>.json source file, for every task in
      TRAIN_TASKS where that source file exists
      adds stage2_factual, stage2_potential; merges relevant_frame_ranges

Resumable: re-running skips a video once every task JSON that exists for it in
<stage1_rag_dir> has a corresponding enriched output in <output_dir>.

Usage
-----
    export MODELSELL_API_KEY=...
    python RAG_stage2.py                              # all videos, all 9 tasks
...
TRAIN_TASKS = [t for t in ALL_TASKS if t != "test"]


# ---------------------------------------------------------------------------
# IO helpers
# ---------------------------------------------------------------------------
def _rel_no_ext(video_id):
    """video_id with its .mp4 extension stripped, directories preserved.

    e.g. "Accident-Bench/land_space/long/videos/000001.mp4" ->
         "Accident-Bench/land_space/long/videos/000001"
    """
    return video_id[:-4] if video_id.endswith(".mp4") else video_id


def _output_path(out_dir, task, video_id):
    """outputdirectory/task/dataset/.../video.json, mirroring aicity_train_2stage."""
    return os.path.join(out_dir, task, _rel_no_ext(video_id) + ".json")
...
def _pending(job):
    """Return True if any task's stage2 enrichment has not yet been written.

    A video is fully done only once every TRAIN_TASKS entry that has a stage1
    source file also has a corresponding enriched output file.
    """
    for task in TRAIN_TASKS:
        src_path = _output_path(AICITY_TRAIN_DIR, task, job["video_id"])
        if not os.path.exists(src_path):
            continue  # this task doesn't apply to this video
        dst_path = _output_path(STAGE2_RAG_DIR, task, job["video_id"])
...
def process_video(job):
    ...
    Saves:
      <output_dir>/_raw_cache/<video_id_no_ext>.json   parsed retrieval fields (resumability marker)
      <output_dir>/<task>/<video_id_no_ext>.json       enriched JSON, for every task in TRAIN_TASKS
          that has a matching stage1 source file
          adds stage2_factual, stage2_potential; merges relevant_frame_ranges
    ...
    try:
        # Raw stage2 cache is task-independent (video-level context only), kept
        # under its own subtree so it never collides with the per-task output tree.
        raw_path = os.path.join(STAGE2_RAG_DIR, "_raw_cache", _rel_no_ext(job["video_id"]) + ".json")
        ...
        # 2. Enrich every task's stage1 RAG JSON that exists for this video and
        #    write to output_dir/<task>/dataset/.../video.json. The retrieval
        #    (factual/potential/frame_ranges) is video-level, so it is reused
        #    identically across all applicable tasks.
        n_updated = 0
        for task in TRAIN_TASKS:
            src_path = _output_path(AICITY_TRAIN_DIR, task, job["video_id"])
            if not os.path.exists(src_path):
        ...
        if n_updated == 0:
            print(f"[warn] no stage1 src found for any task: {job['video_id']}", flush=True)
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Stage 2 RAG: enrich all 9 aicity_train task JSONs per video with "
                     "shared video-level context (factual/potential/frame_ranges)."
    )
    p.add_argument("--media-root", default=DEFAULT_MEDIA_ROOT)
    p.add_argument("--json-dir", default=BCQ_JSON,
                   help="Any one task JSON, used only to enumerate the unique set of "
                        "videos to process (video sets overlap across tasks).")
    p.add_argument("--stage1-rag-dir", default=AICITY_TRAIN_DIR,
                   help="Parent dir containing per-task subfolders, e.g. <dir>/bcq/dataset/.../video.json")
    p.add_argument("--output_dir", default=STAGE2_RAG_DIR,
                   help="Parent dir to write enriched per-task subfolders to, mirroring --stage1-rag-dir.")
...
# Enriches all 9 training tasks (bcq, bcq_openended, causal_linkage, mcq, mcq_openended,
# open_qa, temporal_description, temporal_localization, video_summarization) per video in one pass:
# python RAG_stage2.py --json-dir /workspace/TAU-R1/dataset/train/bcq.json --stage1-rag-dir /data/aicity_train --output_dir /data/aicity_train_2stage_new
#
# For the held-out test set (single "test" task, different layout), use RAG_stage2_train.py --task test instead:
# python RAG_stage2_train.py --json-dir /workspace/TAU-R1/dataset/test/tar_test/test.json --stage1-rag-dir /data/RAG_test_scene/test/tar_test --output_dir /data/RAG_Stage2_test --task test
# python RAG_stage2_train.py --json-dir /workspace/TAU-R1/dataset/test/PSI_VQA/temporal_localization.json --stage1-rag-dir /data/PSI_VQA_test_rag_stage1 --output_dir /data/PSI_VQA_test_rag_stage2